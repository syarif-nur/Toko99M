...
class BarangPagingSource(private val apiService: ApiService, private val keyword: String) :
    PagingSource<Int, ItemBarang>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ItemBarang> {
        return try {
            ...
            val response = apiService.getBarang(page = page, search = keyword)
            LoadResult.Page(
                data = response.data,
